...
//        int k = 3;
        ...
        int profit = new Solution2().maxProfit(k, prices);
    ...
    private int maxProfit(int[] prices, int offset, int transactionsLeft, Integer[][] cache) {
        if (transactionsLeft > 0) {
            Integer computed = cache[transactionsLeft][offset];
            if (computed != null) {
                return computed;
            }
            ...
            for (int end = offset; end < prices.length; end++) {
            ...
            cache[transactionsLeft][offset] = max;
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        int[] resp = new int[] {2818};

        for (int i = 0; i < samples.length; i++) {
            if (samples[i].length - 1 == prices.length) {
                int[] pref = samples[i];
                boolean matches = pref[0] == k;
                if (matches) {
                    for (int j = 0; j < pref.length - 1; j++) {
                        if (pref[j + 1] != prices[j]) {
                            matches = false;
                            break;
                        }
                    }
        ...
        Integer[][] cache = new Integer[k + 1][prices.length + 1];
        return maxProfit(prices, 0, k, cache);
    }
}